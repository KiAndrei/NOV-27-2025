...
// Run automatic cleanup if needed (runs once per day)
$archiveHelper = new MessageArchiveHelper();
$archiveHelper->autoCleanupIfNeeded($conn);
...
if (isset($_POST['action']) && $_POST['action'] === 'fetch_messages') {
    ...
    // Fetch messages from database and archive
    $archiveHelper = new MessageArchiveHelper();
    $allMessages = $archiveHelper->getAllMessages($conversation_id, 'employee', $conn);
    
    foreach ($allMessages as $row) {
        $sent = $row['sender_type'] === 'employee';
        $row['profile_image'] = $sent ? $employee_img : $client_img;
        // Mark if from archive
        if (isset($row['from_archive']) && $row['from_archive']) {
...
<body>
    ...
    <style>
        ...
        .message-bubble.archived {
            opacity: 0.85;
        }
        
        .message-bubble.archived .message-text {
            font-style: italic;
        }
    ...
    <script>
            ...
            fetch('employee_messages.php', { method: 'POST', body: fd })
                .then(r => r.json()).then(msgs => {
                    ...
                    msgs.forEach(m => {
                        ...
                        const isArchived = m.is_archived || m.from_archive;
                        chat.innerHTML += `
                            <div class='message-bubble ${sent ? 'sent' : 'received'} ${isArchived ? 'archived' : ''}'>
                                ...
                                <div class='message-content'>
                                    ...
                                    <div class='message-meta'>
                                        <span>${m.sent_at}</span>
                                        ${isArchived ? '<span class="archived-indicator" title="Archived message"><i class="fas fa-archive"></i></span>' : ''}
                                    </div>
                                </div>
        ...
        const hamburgerOriginalParent = hamburgerMenu ? hamburgerMenu.parentElement : null;
        const hamburgerNextSibling = hamburgerMenu ? hamburgerMenu.nextElementSibling : null;
        const hamburgerPlaceholder = document.createElement('span');

        if (hamburgerMenu && hamburgerOriginalParent) {
            hamburgerPlaceholder.style.display = 'none';
            hamburgerOriginalParent.insertBefore(
                hamburgerPlaceholder,
                hamburgerNextSibling
            );
        }
