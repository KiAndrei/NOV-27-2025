...
// You can set MESSAGE_DURATION_MINUTES directly for fine-grained control (e.g., 1 minute for testing)
if (!defined('MESSAGE_DURATION_MINUTES')) {
    define('MESSAGE_DURATION_MINUTES', 1); // Temporary: 1 minute duration for testing
}
// For backwards compatibility or display purposes, keep DAYS constant in sync
if (!defined('MESSAGE_DURATION_DAYS')) {
    define('MESSAGE_DURATION_DAYS', MESSAGE_DURATION_MINUTES / (24 * 60));
}

// Cleanup interval (how often auto-cleanup runs, in minutes)
// Default: run as often as the duration, but not more than once per day
if (!defined('MESSAGE_CLEANUP_INTERVAL_MINUTES')) {
    $calculatedInterval = max(1, min(MESSAGE_DURATION_MINUTES, 24 * 60)); // between 1 minute and 1 day
    define('MESSAGE_CLEANUP_INTERVAL_MINUTES', $calculatedInterval);
}
?>
