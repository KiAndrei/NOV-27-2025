...
$stmt->execute();
  ...
  if (isset($_POST['action']) && $_POST['action'] === 'edit_event') {
      ...
      // Validate time range (8:00 AM - 10:00 PM, exact limit)
      $startMinutes = intval(substr($start_time, 0, 2)) * 60 + intval(substr($start_time, 3, 2));
      $endMinutes = intval(substr($end_time, 0, 2)) * 60 + intval(substr($end_time, 3, 2));
      
      if ($startMinutes < 480 || $startMinutes >= 1320) {
          echo 'error: You can only schedule between 8:00 AM and 10:00 PM.';
          exit;
      }
      
      if ($endMinutes <= 480 || $endMinutes > 1320) {
          echo 'error: End time cannot go past exactly 10:00 PM.';
          exit;
      }
...
if (isset($_POST['action']) && $_POST['action'] === 'add_event') {
    ...
    // Validate time window (8:00 AM - 10:00 PM exact)
    $startMinutes = intval(substr($start_time, 0, 2)) * 60 + intval(substr($start_time, 3, 2));
    $endMinutes = intval(substr($end_time, 0, 2)) * 60 + intval(substr($end_time, 3, 2));
...
<body>
            ...
            <div class="modal-body">
                <form id="eventForm" class="event-form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="time" id="eventStartTime" name="start_time" min="08:00" max="22:00">
                    </div>
                    <div class="form-group">
            ...
            <div class="modal-body">
                <form id="editEventForm" class="event-form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="time" id="editEventStartTime" name="start_time" required min="08:00" max="22:00">
                    </div>
                    <div class="form-group">
    ...
    <script>
        ...
        // Shared time-window helpers for 8:00 AM - 10:00 PM rules
        ...
        function setupAttorneyAddScheduleValidation() {
            ...
            if (startTimeInput) {
                startTimeInput.addEventListener('change', function() {
                    ...
                    if (startMinutes !== null && endTimeInput) {
        ...
        // Validate time range for attorney add schedule (8AM-10PM exact)
        function validateAttorneyAddTimeRange() {
            ...
            if (startMinutes !== null && !isStartTimeValid(startMinutes)) {
                showNotification(`❌ ${TIME_RANGE_ERROR}`, 'error');
                return false;
            }
        ...
        // Validate time range for attorney (8AM-10PM exact)
        ...
        window.saveEventChanges = async function() {
            ...
            // Validate time range (8:00 AM - 10:00 PM exact)
            const startMinutes = getMinutesFromTime(startTime);
            if (startMinutes === null || !isStartTimeValid(startMinutes)) {
                showNotification(`❌ ${TIME_RANGE_ERROR} Cannot save with invalid start time.`, 'error');
            ...
            const endMinutes = getMinutesFromTime(endTime);
            if (endMinutes === null || !isEndTimeValid(endMinutes)) {
                showNotification(`❌ ${TIME_RANGE_ERROR} ${END_LIMIT_ERROR} Cannot save with invalid end time.`, 'error');
