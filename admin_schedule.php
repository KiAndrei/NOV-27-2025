...
if (isset($_POST['action']) && $_POST['action'] === 'edit_event') {
    ...
    $endMinute = intval(substr($end_time, 3, 2));
...
foreach ($events as $ev) {
    ...
    $js_events[] = [
        ...
        'extendedProps' => [
            ...
            'description' => $ev['description'] ?? '-',
            'client' => $ev['client_name'] ?? '-',
            'createdBy' => $ev['created_by_name'] ?? '-',
            'walkinClientName' => $ev['walkin_client_name'] ?? null,
            'walkinClientContact' => $ev['walkin_client_contact'] ?? null
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (saveEventBtn) {
                ...
                const endMinute = parseInt(eventEndTime.split(':')[1], 10);
                
                if (startHour < 8 || startHour >= 22) {
                    showNotification('❌ You can only schedule between 8:00 AM and 10:00 PM. Cannot save with invalid start time.', 'error');
                    document.getElementById('eventStartTime').focus();
                    return;
                }
                
                if (endHour < 8 || endHour > 22 || (endHour === 22 && endMinute > 0)) {
                    showNotification('❌ End time cannot go past exactly 10:00 PM.', 'error');
                    document.getElementById('eventEndTime').focus();
                    return;
                }
        ...
        function validateAdminAddTimeRange() {
            ...
            const startMinute = parseInt(startTime.split(':')[1], 10);
                if (startHour < 8 || startHour >= 22) {
                    showNotification('❌ You can only schedule between 8:00 AM and 10:00 PM.', 'error');
                    return false;
                }
            }
        ...
        function initializeEditEventHandlers() {
            ...
            if (saveEditEvent) {
                ...
                saveEditEvent.addEventListener('click', async function() {
                    ...
                    if (endHour < 8 || endHour > 22 || (endHour === 22 && endMinute > 0)) {
                        showNotification('❌ End time cannot go past exactly 10:00 PM. Cannot save with invalid end time.', 'error');
