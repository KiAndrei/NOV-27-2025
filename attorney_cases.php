...
if (isset($_POST['action']) && $_POST['action'] === 'add_case') {
    ...
    // Ensure client has an approved access request before creating a case
    $approvalStmt = $conn->prepare("SELECT id FROM client_request_form WHERE client_id = ? AND status = 'Approved' ORDER BY reviewed_at DESC, submitted_at DESC LIMIT 1");
    if (!$approvalStmt) {
        echo 'error: Unable to verify client approval status.';
        exit();
    }
    $approvalStmt->bind_param('i', $client_id);
    $approvalStmt->execute();
    $approvalRes = $approvalStmt->get_result();
    if (!$approvalRes || !$approvalRes->fetch_assoc()) {
        echo 'error: Client request access is not yet approved. Please wait for employee verification before creating a case.';
        exit();
    }
    ...
    <script>
        ...
        function openDocumentUpload(caseId) {
                    ...
                    <div class="modal-body">
                        <form id="documentUploadForm" enctype="multipart/form-data">
                            ...
                            <div class="upload-area" onclick="document.getElementById('fileInput').click()">
                                <input type="file" id="fileInput" name="documents[]" multiple accept=".pdf,.jpg,.jpeg,.png,.gif,.webp" style="display: none;">
                                <div class="upload-content">
                                    <i class="fas fa-cloud-upload-alt"></i>
                                <h3>Click to select files</h3>
                                <p>Only PDF or image files (JPG, PNG, GIF, WEBP) are supported</p>
            ...
            document.getElementById('fileInput').addEventListener('change', function(e) {
                ...
                const allowedMimeTypes = ['application/pdf','image/jpeg','image/png','image/gif','image/webp','image/pjpeg','image/x-png'];
                ...
                Array.from(e.target.files).forEach((file, index) => {
                    ...
                    if (file.size > maxSize) {
                        ...
                        perFileErrors.push('Exceeds 10MB limit.');
                    }
                    
                    const extension = file.name.split('.').pop().toLowerCase();
                    const isAllowedType = allowedMimeTypes.includes(file.type) || allowedExtensions.includes(extension);
                    if (!isAllowedType) {
                        hasErrors = true;
                        errorMessage += `• ${file.name} - unsupported file type<br>`;
                        fileItem.style.borderLeft = '4px solid #dc3545';
                        fileItem.style.backgroundColor = '#f8d7da';
                        perFileErrors.push('Unsupported file type. Only PDF or image files are allowed.');
                    }
                    ...
                    if (perFileErrors.length) {
                        ...
                        warning.innerHTML = perFileErrors.join(' ');
                ...
                if (hasErrors) {
                    fileSizeError.innerHTML = `<strong>Upload blocked:</strong><br>${errorMessage || 'Please review the highlighted files.'}`;
