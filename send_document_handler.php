...
if ($stmt->execute()) {
    ...
    // Notify employees about the new document generation request
    notifyEmployeesAboutDocumentRequest(
        $conn,
        $client_id,
        $client_name,
        $document_type,
...
/**
 * Notify all employees that a client submitted a new document generation request.
 *
 * @param mysqli $conn
 * @param int $clientId
 ...
 * @return void
 */
function notifyEmployeesAboutDocumentRequest($conn, $clientId, $clientName, $documentType, $requestId) {
    if (!$conn) {
        return;
    }
    ...
    try {
        ...
        error_log("Document Handler Debug - Notification table check failed: " . $th->getMessage());
        return;
    }
    ...
    $documentLabel = getDocumentTypeDisplayName($documentType);
    $displayName = $clientName ?: ('Client ID: ' . $clientId);
    $title = 'New Document Generation Request';
    $message = $displayName . " submitted a new {$documentLabel} document request (ID: {$requestId}). Please review it in Document Generation.";
    
    $employeesStmt = $conn->prepare("SELECT id FROM user_form WHERE user_type = 'employee'");
    if (!$employeesStmt) {
        error_log("Document Handler Debug - Failed to prepare employee lookup: " . $conn->error);
        return;
    }
    
    if (!$employeesStmt->execute()) {
        error_log("Document Handler Debug - Failed to execute employee lookup: " . $employeesStmt->error);
        $employeesStmt->close();
        return;
    }
    ...
    $notifyStmt = $conn->prepare("INSERT INTO notifications (user_id, user_type, title, message, type) VALUES (?, 'employee', ?, ?, 'info')");
    if (!$notifyStmt) {
        error_log("Document Handler Debug - Failed to prepare notification insert: " . $conn->error);
        $employeesStmt->close();
        return;
    }
    ...
    $titleText = $title;
    $messageText = $message;
    $notifyStmt->bind_param('iss', $employeeId, $titleText, $messageText);
    
    while ($employee = $employeesResult->fetch_assoc()) {
        $employeeId = (int)$employee['id'];
        if (!$notifyStmt->execute()) {
            error_log("Document Handler Debug - Failed to insert notification for employee {$employeeId}: " . $notifyStmt->error);
        }
    }
...
/**
 * Convert internal document type keys into human-readable labels.
 *
 * @param string $documentType
 * @return string
 */
function getDocumentTypeDisplayName($documentType) {
    $map = [
        ...
        'pwdLoss' => 'PWD ID Loss',
        'boticabLoss' => 'Boticab Loss',
        'seniorIDLoss' => 'Senior ID Loss',
        'jointAffidavit' => 'Joint Affidavit (Two Disinterested Persons)',
        'jointAffidavitSoloParent' => 'Joint Affidavit (Solo Parent)',
        'swornAffidavitMother' => 'Sworn Affidavit of Mother',
        'swornAffidavitSoloParent' => 'Sworn Affidavit (Solo Parent)'
    ];
    ...
    $formatted = preg_replace('/([a-z])([A-Z])/', '$1 $2', $documentType);
    $formatted = str_replace(['_', '-'], ' ', $formatted);
    $formatted = preg_replace('/\s+/', ' ', $formatted);
    
    return ucwords(trim($formatted ?: 'Document'));
}
