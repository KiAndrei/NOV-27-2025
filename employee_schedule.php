...
if (isset($_POST['action']) && $_POST['action'] === 'add_event') {
    ...
    $startMinutes = intval(substr($start_time, 3, 2));
    $endHour = intval(substr($end_time, 0, 2));
    $endMinutes = intval(substr($end_time, 3, 2));
    $startTotalMinutes = ($startHour * 60) + $startMinutes;
    $endTotalMinutes = ($endHour * 60) + $endMinutes;
    $startLimitMinutes = 8 * 60;   // 8:00 AM
    $endLimitMinutes = 22 * 60;    // 10:00 PM
    
    if ($startTotalMinutes < $startLimitMinutes || $startTotalMinutes >= $endLimitMinutes) {
        echo 'error: You can only schedule between 8:00 AM and 10:00 PM. Start time must be before 10:00 PM.';
        exit;
    }
    
    if ($endTotalMinutes <= $startLimitMinutes || $endTotalMinutes > $endLimitMinutes) {
        echo 'error: You can only schedule between 8:00 AM and 10:00 PM. End time cannot go past exactly 10:00 PM.';
        exit;
    }
    ...
    $startTotalMinutes = ($startHour * 60) + $startMinutes;
    $endTotalMinutes = ($endHour * 60) + $endMinutes;
    $startLimitMinutes = 8 * 60;
    $endLimitMinutes = 22 * 60;
...
if (isset($_POST['action']) && $_POST['action'] === 'edit_event') {
    ...
    // Validate time range (8AM-10PM, end cannot exceed 10PM)
    $startHour = intval(substr($start_time, 0, 2));
    $startMinutes = intval(substr($start_time, 3, 2));
    $endHour = intval(substr($end_time, 0, 2));
    $endMinutes = intval(substr($end_time, 3, 2));
...
<body>
            ...
            <div class="modal-body">
                <form id="eventForm" class="event-form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <input type="time" id="eventStartTime" name="start_time" min="08:00" max="21:59">
                        </div>
                        <div class="form-group">
                            <label for="eventEndTime" style="color: #dc3545;">End Time *</label>
                            <input type="time" id="eventEndTime" name="end_time" min="08:00" max="22:00">
                        </div>
                    <div class="form-group">
            ...
            <div class="modal-body">
                <form id="editEventForm" class="event-form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="time" id="editEventStartTime" name="start_time" required min="08:00" max="21:59">
                    </div>
                    <div class="form-group">
                        <label for="editEventEndTime">End Time</label>
                        <input type="time" id="editEventEndTime" name="end_time" required min="08:00" max="22:00">
                    </div>
                    <div class="form-group">
            ...
            <div class="modal-body">
                <form id="walkinForm" class="event-form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="time" id="walkinStartTime" name="start_time" required min="08:00" max="21:59">
                    </div>
                    <div class="form-group">
                        <label for="walkinEndTime" style="color: #dc3545;">End Time *</label>
                        <input type="time" id="walkinEndTime" name="end_time" required min="08:00" max="22:00">
                    </div>
                    <div class="form-group">
    ...
    <script>
        ...
        const START_LIMIT_MINUTES = 8 * 60;
        const END_LIMIT_MINUTES = 22 * 60;
        const TIME_RANGE_ERROR = 'You can only schedule between 8:00 AM and 10:00 PM.';
        const END_LIMIT_ERROR = 'End time cannot go past exactly 10:00 PM.';
        ...
        function isStartTimeValid(minutes) {
            return minutes !== null && minutes >= START_LIMIT_MINUTES && minutes < END_LIMIT_MINUTES;
        }
        
        function isEndTimeValid(minutes) {
            return minutes !== null && minutes > START_LIMIT_MINUTES && minutes <= END_LIMIT_MINUTES;
        }
        ...
        function initializeCalendarFunctions(calendar) {
            ...
            if (saveEventBtn) {
                saveEventBtn.onclick = function() {
                    ...
                    // Validate time range (8AM-10PM) - STRICT VALIDATION
                    const startMinutes = getMinutesFromTime(eventStartTime);
                    const endMinutes = getMinutesFromTime(eventEndTime);
                    
                    if (!isStartTimeValid(startMinutes)) {
                        showNotification(`❌ ${TIME_RANGE_ERROR} Start time must be before 10:00 PM.`, 'error');
                        eventStartTimeEl.focus();
                        return;
                    }
                    
                    if (!isEndTimeValid(endMinutes)) {
                        showNotification(`❌ ${TIME_RANGE_ERROR} ${END_LIMIT_ERROR}`, 'error');
                        eventEndTimeEl.focus();
                        return;
                    }
            ...
            if (saveWalkinBtn) {
                ...
                // Validate time range (8AM-10PM) - STRICT VALIDATION
                const walkinStartMinutes = getMinutesFromTime(walkinStartTime);
                const walkinEndMinutes = getMinutesFromTime(walkinEndTime);
                
                if (!isStartTimeValid(walkinStartMinutes)) {
                    showNotification(`❌ ${TIME_RANGE_ERROR} Start time must be before 10:00 PM.`, 'error');
                    document.getElementById('walkinStartTime').focus();
                    return;
                }
                
                if (!isEndTimeValid(walkinEndMinutes)) {
                    showNotification(`❌ ${TIME_RANGE_ERROR} ${END_LIMIT_ERROR}`, 'error');
                    document.getElementById('walkinEndTime').focus();
                    return;
                }
        ...
        function setupWalkinScheduleValidation() {
            ...
            // Automatic end time (30-minute interval, capped at 10:00 PM)
            if (startTimeInput) {
                startTimeInput.addEventListener('change', function() {
                    if (endTimeInput) {
                        const startMinutes = getMinutesFromTime(this.value);
                        if (startMinutes !== null) {
                            let suggestedEnd = startMinutes + 30;
                            if (suggestedEnd > END_LIMIT_MINUTES) {
                                suggestedEnd = END_LIMIT_MINUTES;
                            }
                            endTimeInput.value = minutesToTimeString(suggestedEnd);
                        }
                    }
                });
            }

            // Time range validation (8AM-10PM)
        ...
        // Validate time range for walk-in schedule (8AM-10PM)
        function validateWalkinTimeRange() {
            const startMinutes = getMinutesFromTime(document.getElementById('walkinStartTime').value);
            const endMinutes = getMinutesFromTime(document.getElementById('walkinEndTime').value);
            
            if (startMinutes !== null && !isStartTimeValid(startMinutes)) {
                showNotification(`❌ ${TIME_RANGE_ERROR} Start time must be before 10:00 PM.`, 'error');
                return false;
            }
            
            if (endMinutes !== null && !isEndTimeValid(endMinutes)) {
                showNotification(`❌ ${TIME_RANGE_ERROR} ${END_LIMIT_ERROR}`, 'error');
                return false;
            }
        ...
        // Validate time range for add schedule (8AM-10PM)
        function validateAddTimeRange() {
            const startMinutes = getMinutesFromTime(document.getElementById('eventStartTime').value);
            const endMinutes = getMinutesFromTime(document.getElementById('eventEndTime').value);
        ...
        // Validate time range for edit schedule (8AM-10PM)
        function validateEditTimeRange() {
            const startMinutes = getMinutesFromTime(document.getElementById('editEventStartTime').value);
            const endMinutes = getMinutesFromTime(document.getElementById('editEventEndTime').value);
        ...
        async function saveEventChanges() {
            ...
            // Validate time range (8AM-10PM)
            const editStartMinutes = getMinutesFromTime(startTime);
            const editEndMinutes = getMinutesFromTime(endTime);
            
            if (!isStartTimeValid(editStartMinutes)) {
                showNotification(`❌ ${TIME_RANGE_ERROR} Start time must be before 10:00 PM.`, 'error');
                document.getElementById('editEventStartTime').focus();
                return;
            }
            
            if (!isEndTimeValid(editEndMinutes)) {
                showNotification(`❌ ${TIME_RANGE_ERROR} ${END_LIMIT_ERROR}`, 'error');
