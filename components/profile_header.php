...
<script>
    window.profileHeaderUserType = <?= json_encode(strtolower($_SESSION['user_type'] ?? '')); ?>;
</script>
...
<div class="header" style="padding: 12px 20px; min-height: auto;">
    <div class="header-title">
        ...
        <p class="header-subtitle" style="font-size: 0.85rem; margin: 4px 0 0 0; opacity: 0.8;"><?= $page_subtitle ?? 'Overview of your activities' ?></p>
    </div>
                    ...
                    <div class="profile-edit-container">
                        <form method="POST" enctype="multipart/form-data" class="profile-form" id="editProfileForm">
                            ...
                            <div class="form-section" style="margin-bottom: 14px;">
                                ...
                                <div class="form-group" style="margin-bottom: 10px;">
                                    <label for="profile_email" style="display: block; margin-bottom: 4px; color: #333; font-weight: 500; font-size: 0.8rem;">Email Address</label>
                                    <input type="email" id="profile_email" name="email" value="<?= htmlspecialchars($user_email ?? '') ?>" readonly style="background-color: #f5f5f5; cursor: not-allowed; width: 100%; padding: 6px 10px; border: 2px solid #e1e5e9; border-radius: 6px; font-size: 0.8rem;">
    ...
    <div class="modal-content" style="background-color: white; padding: 0; border-radius: 12px; width: 90%; max-width: 450px; max-height: 90vh; overflow-y: auto; box-shadow: 0 20px 60px rgba(0, 0, 0, 0.3);">
        ...
        <div class="modal-body" style="padding: 25px;">
            <div style="text-align: center; margin-bottom: 25px;">
                ...
                <h3 style="margin: 0; color: #333; font-size: 1.2rem;">Secure Password Change</h3>
                <p style="margin: 10px 0 0 0; color: #666; font-size: 1rem;">We'll send a verification code to your email address</p>
            </div>
            
            <form id="changePasswordForm">
                <div class="form-group" style="margin-bottom: 20px;">
                    <label for="change_password_email" style="display: block; margin-bottom: 8px; color: #333; font-weight: 500; font-size: 1rem;">Email Address</label>
                    <input type="email" id="change_password_email" name="email" value="<?= htmlspecialchars($user_email ?? '') ?>" readonly style="background-color: #f5f5f5; cursor: not-allowed; width: 100%; padding: 14px 16px; border: 2px solid #e1e5e9; border-radius: 6px; font-size: 1rem;">
                    <small style="color: #666; font-size: 13px; margin-top: 5px; display: block;">Verification code will be sent to this email</small>
                </div>
                
                <div class="form-actions" style="display: flex; gap: 10px; justify-content: center; margin-top: 25px;">
                    <button type="submit" id="sendCodeBtn" class="btn btn-primary" style="padding: 14px 24px; border-radius: 8px; text-decoration: none; font-weight: 600; cursor: pointer; border: none; display: inline-flex; align-items: center; justify-content: center; gap: 8px; transition: all 0.3s ease; font-size: 1.05rem; background: linear-gradient(135deg, #5D0E26, #8B1538); color: white; width: 100%; box-shadow: 0 4px 12px rgba(93, 14, 38, 0.3);">
...
<div id="otpExpiredModal" style="display: none; position: fixed; z-index: 999999; left: 0; top: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.85); backdrop-filter: blur(8px); align-items: center; justify-content: center;">
    <div style="background: white; border-radius: 16px; width: 90%; max-width: 450px; box-shadow: 0 20px 60px rgba(0, 0, 0, 0.3); overflow: hidden; animation: modalSlideIn 0.3s ease-out;">
        <!-- Header -->
...
<div id="invalidOtpModal" style="display: none; position: fixed; z-index: 999999; left: 0; top: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.85); backdrop-filter: blur(8px); align-items: center; justify-content: center;">
    <div style="background: white; border-radius: 16px; width: 90%; max-width: 450px; box-shadow: 0 20px 60px rgba(0, 0, 0, 0.3); overflow: hidden; animation: modalSlideIn 0.3s ease-out;">
        ...
        <!-- Body -->
        <div style="padding: 24px;">
            <p style="margin: 0 0 12px 0; font-size: 1.05rem; color: #333; line-height: 1.6; font-weight: 500;" id="invalidOtpMessage">
...
const notificationRouteMap = {
    admin: {
        ...
        cases: 'admin_managecases.php',
        documents: 'admin_documents.php',
        document_generation: 'admin_document_generation.php',
        requests: 'admin_clients.php',
        clients: 'admin_clients.php'
    },
    attorney: {
        default: 'attorney_dashboard.php',
        messages: 'attorney_messages.php',
        schedule: 'attorney_schedule.php',
        cases: 'attorney_cases.php',
        documents: 'attorney_documents.php',
        document_generation: 'attorney_document_generation.php',
        requests: 'attorney_clients.php',
        clients: 'attorney_clients.php'
    },
    employee: {
        default: 'employee_dashboard.php',
        messages: 'employee_messages.php',
        schedule: 'employee_schedule.php',
        cases: 'employee_clients.php',
        documents: 'employee_document_handler.php',
        document_generation: 'employee_send_files.php',
        requests: 'employee_request_management.php',
        clients: 'employee_clients.php'
    },
    client: {
        default: 'client_dashboard.php',
        messages: 'client_messages.php',
        schedule: 'client_schedule.php',
        cases: 'client_cases.php',
        documents: 'client_documents.php',
        document_generation: 'client_documents.php',
        requests: 'client_request_form.php',
        clients: 'client_cases.php'
    }
};
...
function getRouteForCategory(userType, category) {
    const map = notificationRouteMap[userType] || {};
    return map[category] || map.default || 'index.php';
}
...
function getNotificationUrl(notification) {
    ...
    const notificationUserType = (notification?.user_type || '').toLowerCase();
    const sessionUserType = (typeof window !== 'undefined' && window.profileHeaderUserType)
        ? String(window.profileHeaderUserType).toLowerCase()
        : '';
    const userType = sessionUserType || notificationUserType;
    ...
    if (title.includes('verification code') || message.includes('verification code')) {
        return 'change_password.php';
    }
    
    if (title.includes('message') || message.includes('message')) {
        return getRouteForCategory(userType, 'messages');
    }
    
    if (
        title.includes('document generation') ||
        message.includes('document generation') ||
        title.includes('document request') ||
        message.includes('document request')
    ) {
        return getRouteForCategory(userType, 'document_generation');
    }
    
    if (title.includes('new client request') || message.includes('messaging request')) {
        return getRouteForCategory(userType, 'requests');
    }
    
    if (title.includes('request approved')) {
        return getRouteForCategory(userType, 'messages');
    }
    
    if (title.includes('request rejected')) {
        return getRouteForCategory(userType, 'requests');
    }
    ...
    if (title.includes('schedule') || message.includes('schedule') || title.includes('appointment')) {
        return getRouteForCategory(userType, 'schedule');
    }
    
    if (title.includes('new client assignment')) {
        return getRouteForCategory(userType, 'messages');
    }
    
    if (title.includes('assignment') || message.includes('assigned')) {
        return getRouteForCategory(userType, 'messages');
    }
    
    if (title.includes('case status') || message.includes('case status')) {
        return getRouteForCategory(userType, 'cases');
    }
    
    if (title.includes('case') || message.includes('case')) {
        return getRouteForCategory(userType, 'cases');
    }
    
    if (title.includes('document') || message.includes('document') || title.includes('file')) {
        return getRouteForCategory(userType, 'documents');
    }
    
    if (title.includes('profile') || message.includes('profile')) {
        return 'update_profile.php';
    }
    
    if (title.includes('password') || message.includes('password')) {
        return 'change_password.php';
    }
    
    return getRouteForCategory(userType, 'default');
}

let notificationListHandlersAttached = false;

function ensureNotificationListHandlers() {
    if (notificationListHandlersAttached) {
        return;
    }
    ...
    notificationListHandlersAttached = true;
}
            ...
            <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 10px;">
                <div style="flex: 1;">
                    <div style="font-weight: 600; font-size: 14px; color: #374151; margin-bottom: 4px; display: flex; align-items: center; gap: 6px;">
                        ${safeTitle}
                        ${hasUrl ? '<i class="fas fa-external-link-alt" style="font-size: 10px; color: #6b7280;"></i>' : ''}
                    </div>
                    <div style="font-size: 12px; color: #6b7280; margin-bottom: 4px;">${safeMessage}</div>
                    <div style="font-size: 11px; color: #9ca3af;">${formatTime(notification.created_at)}</div>
                </div>
                <div class="notification-dot" style="width: 8px; height: 8px; border-radius: 50%; background: ${getNotificationColor(notification.type)}; ${notification.is_read ? 'display: none;' : ''}"></div>
            </div>
        </div>`;
    }).join('');
    
    ensureNotificationListHandlers();
}
...
function markOneAsRead(id) {
    ...
    if (notificationElement) {
        ...
        // Hide the blue dot
        const blueDot = notificationElement.querySelector('.notification-dot');
        if (blueDot) {
    ...
    .then(data => {
        if (data.success) {
            ...
            // If response doesn't have success, still reload to be safe
            loadNotifications();
        }
    })
    .catch(error => {
        console.error('Error marking notification as read:', error);
        // Fallback: reload all notifications
        loadNotifications();
    });
}
