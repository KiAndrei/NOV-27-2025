...
if (isset($_POST['action']) && $_POST['action'] === 'fetch_messages') {
    ...
    $archiveHelper = new MessageArchiveHelper();
    $allMessages = $archiveHelper->getAllMessages($conversation_id, 'attorney', $conn);
            ...
            fetch('admin_messages.php', { method: 'POST', body: fd })
                .then(r => r.json()).then(msgs => {
                    ...
                    msgs.forEach(m => {
                        ...
                        const isUnseen = !sent && !isArchived && (Number(m.is_seen) === 0);
                        if (isUnseen) hasUnseenMessages = true;
                        chat.innerHTML += `
                            <div class='message-bubble ${sent ? 'sent' : 'received'} ${isUnseen ? 'unseen' : ''} ${isArchived ? 'archived' : ''}'>
